...
class CVRSystem_017 : public IVRSystem_017::IVRSystem, public CVRCommon {
	CVR_GEN_IFACE();
...
public:
	...
	*/
	INTERFACE_FUNC(void, GetOutputDevice, uint64_t *pnDevice, ETextureType textureType, VkInstance_T *pInstance = nullptr);
	...
	* in the list, or the size of the array needed if not large enough. */
	...
	INTERFACE_FUNC(EDeviceActivityLevel, GetTrackedDeviceActivityLevel, vr::TrackedDeviceIndex_t unDeviceId);
	...
	* actual tracked device. */
	INTERFACE_FUNC(ETrackedDeviceClass, GetTrackedDeviceClass, vr::TrackedDeviceIndex_t unDeviceIndex);

	/** Returns true if there is a device connected in this slot. */
	INTERFACE_FUNC(bool, IsTrackedDeviceConnected, vr::TrackedDeviceIndex_t unDeviceIndex);

	/** Returns a bool property. If the device index is not valid or the property is not a bool type this function will return false. */
	INTERFACE_FUNC(bool, GetBoolTrackedDeviceProperty, vr::TrackedDeviceIndex_t unDeviceIndex, ETrackedDeviceProperty prop, ETrackedPropertyError *pErrorL);

	/** Returns a float property. If the device index is not valid or the property is not a float type this function will return 0. */
	INTERFACE_FUNC(float, GetFloatTrackedDeviceProperty, vr::TrackedDeviceIndex_t unDeviceIndex, ETrackedDeviceProperty prop, ETrackedPropertyError *pErrorL);

	/** Returns an int property. If the device index is not valid or the property is not a int type this function will return 0. */
	INTERFACE_FUNC(int32_t, GetInt32TrackedDeviceProperty, vr::TrackedDeviceIndex_t unDeviceIndex, ETrackedDeviceProperty prop, ETrackedPropertyError *pErrorL);

	/** Returns a uint64 property. If the device index is not valid or the property is not a uint64 type this function will return 0. */
	INTERFACE_FUNC(uint64_t, GetUint64TrackedDeviceProperty, vr::TrackedDeviceIndex_t unDeviceIndex, ETrackedDeviceProperty prop, ETrackedPropertyError *pErrorL);

	/** Returns a matrix property. If the device index is not valid or the property is not a matrix type, this function will return identity. */
	INTERFACE_FUNC(HmdMatrix34_t, GetMatrix34TrackedDeviceProperty, vr::TrackedDeviceIndex_t unDeviceIndex, ETrackedDeviceProperty prop, ETrackedPropertyError *pErrorL);

	/** Returns a string property. If the device index is not valid or the property is not a string type this function will
	* return 0. Otherwise it returns the length of the number of bytes necessary to hold this string including the trailing
	* null. Strings will always fit in buffers of k_unMaxPropertyStringSize characters. */
	INTERFACE_FUNC(uint32_t, GetStringTrackedDeviceProperty, vr::TrackedDeviceIndex_t unDeviceIndex, ETrackedDeviceProperty prop, VR_OUT_STRING() char *pchValue, uint32_t unBufferSize, ETrackedPropertyError *pErrorL);
	...
	* This pose will always be older than the call to this function and should not be used to render the device.
	...
	INTERFACE_FUNC(bool, PollNextEventWithPose, ETrackingUniverseOrigin eOrigin, VREvent_t *pEvent, uint32_t uncbVREvent, vr::TrackedDevicePose_t *pTrackedDevicePose);
	...
	* is invalid. */
	INTERFACE_FUNC(bool, GetControllerState, vr::TrackedDeviceIndex_t unControllerDeviceIndex, vr::VRControllerState_t *pControllerState, uint32_t unControllerStateSize);

	/** fills the supplied struct with the current state of the controller and the provided pose with the pose of
	* the controller when the controller state was updated most recently. Use this form if you need a precise controller
	* pose as input to your application when the user presses or releases a button. */
	INTERFACE_FUNC(bool, GetControllerStateWithPose, ETrackingUniverseOrigin eOrigin, vr::TrackedDeviceIndex_t unControllerDeviceIndex, vr::VRControllerState_t *pControllerState, uint32_t unControllerStateSize, TrackedDevicePose_t *pTrackedDevicePose);

	/** Trigger a single haptic pulse on a controller. After this call the application may not trigger another haptic pulse on this controller
	* and axis combination for 5ms. */
	INTERFACE_FUNC(void, TriggerHapticPulse, vr::TrackedDeviceIndex_t unControllerDeviceIndex, uint32_t unAxisId, unsigned short usDurationMicroSec);
	...
	* The size of the response including its terminating null is returned. */
	INTERFACE_FUNC(uint32_t, DriverDebugRequest, vr::TrackedDeviceIndex_t unDeviceIndex, const char *pchRequest, char *pchResponseBuffer, uint32_t unResponseBufferSize);

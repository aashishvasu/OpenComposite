...
public:
	...
	bool TrackedDeviceAdded(OCTrackedDeviceDriver* driver);

	/** Notifies the server that a tracked device's pose has been updated */
	void TrackedDevicePoseUpdated(uint32_t unWhichDevice, const OCDriverPose_t& newPose, uint32_t unPoseStructSize);
